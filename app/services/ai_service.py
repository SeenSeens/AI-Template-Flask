...
class AiService:
    ...
    """def get_models(self, provider_name: str, api_key: str):
        if provider_name == "OpenAI":
            ...
            from langchain_google_genai import ChatGoogleGenerativeAI
            return ["gemini-pro", "gemini-1.5-pro"]  # Tạm hardcode

        elif provider_name == "Anthropic":
            return ["claude-3-opus", "claude-3-sonnet"]

        raise Exception("Không hỗ trợ provider này.")"""

    """def load_models_from_provider(self, provider_name, api_key):
        if provider_name == "OpenAI":
            from langchain_community.chat_models import ChatOpenAI

            try:
                # Test call để xác thực key
                llm = ChatOpenAI(api_key=api_key, model="gpt-3.5-turbo")
                # Nếu không lỗi thì trả về danh sách models
                return ["gpt-3.5-turbo", "gpt-4"]
            except Exception:
                raise Exception("API key không hợp lệ với OpenAI")
            ...
            try:
                ...
                raise Exception("API key không hợp lệ với Google (Gemini)")

        elif provider_name == "Anthropic":
            from langchain_community.chat_models import ChatAnthropic

            try:
                llm = ChatAnthropic(api_key=api_key, model="claude-3-opus")
                return ["claude-3-opus", "claude-3-sonnet"]
            except Exception:
                raise Exception("API key không hợp lệ với Anthropic")
